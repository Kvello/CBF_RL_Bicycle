...
from torchrl.envs import GymLikeEnv, default_info_dict_reader
...
class custom_terminal_obs_reader(default_info_dict_reader):
    ...
    def __call__(self, info_dict, tensordict):
        ...
        # don't convert info_dict to a tensordict
        ####### THIS IS THE ONLY CHANGE #######
...
class custom_AsyncMeta(_EnvPostInit):
    def __call__(cls, *args, **kwargs):
        instance: custom_GymWrapper = super().__call__(*args, **kwargs)
        ...
        if instance._is_batched:
            ...
            if add_info_dict:
                ...
                instance.auto_register_info_dict(
                    info_dict_reader=custom_terminal_obs_reader(
...
class custom_GymWrapper(GymLikeEnv, metaclass=custom_AsyncMeta):
    """Custom GymWrapper class to handle the terminal observation in a batched environment.
    This is copy-pasted from the original GymWrapper class in torchrl.envs, it just uses the
    custom_terminal_obs_reader class instead of the original one.
    """
...
class custom_GymEnv(custom_GymWrapper):
    """Custom GymEnv class to handle the terminal observation in a batched environment.
    This is copy-pasted from the original GymEnv class in torchrl.envs, it just uses the
    custom_GymWrapper class instead of the original one.
    """
